...
let handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
  ...
  const getDiskUsage = async (path = '/') => {
    try {
      ...
      return { used: '0.00', total: '0.00' };
    }
  };

  const { used, total: totalDisk } = await getDiskUsage('/');
...
╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈꒱`;
  ...
  const textoSubbots = total === 0
    ? '𝙉𝙤 𝙝𝙖𝙮 𝙎𝙪𝙗-𝘽𝙤𝙩𝙨 𝙖𝙘𝙩𝙞𝙫𝙤𝙨 𝙥𝙤𝙧 𝙖𝙝𝙤𝙧𝙖. 🌙'
    : `*✦ Sockets Activos de Sukuna Ultra-MD ✦*

> ⌛ *Tiempo Activo:* _[ ${uptime} ]_
> 🌳 *Sessions Libres:* _[ ${disponibles} ]_
> 🎄 *Subs conectados:* _[ ${total} ]_
> 💾 *Tan usada:* _[ ${used} GB / ${totalDisk} GB ]_
